...
# Paths to original large images and masks
...
def load_rgb_tif(filepath):
    with rasterio.open(filepath) as src:
        img = src.read([1, 2, 3])  # 读取 RGB 三个波段，返回是 [C, H, W]
        img = np.transpose(img, (1, 2, 0))  # 转成 [H, W, C] 格式
    return img.astype(np.uint8)

# Create tiles from large images
...
# Visualization of 2 random tiles
model.eval()
